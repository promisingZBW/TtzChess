...
按起手棋子把棋路摊开：炮、马、车、相、兵各一个中心。上面一排按钮切换开局，下面整屏画一个——点中心的棋子图标新建一条棋路；点已经长出来的小球，直接进去继续打谱。
...
「反转视角」随时对调红黑，两侧路数跟着翻（黑方永远是 1-9，红方永远是一-九）。开局棋路还能用「输出棋谱树」把整棵树存成一张图片。走子、前进后退、点光球跳局面都有落子声，右边喇叭图标可以静音。一方的将被吃掉时，标题旁边会标出谁赢了。
