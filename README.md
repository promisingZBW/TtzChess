...
按起手棋子把棋路摊开，炮、马、车、相、兵各一个。上面一排按钮切换开局，下面整屏只画一个，谱名再长也铺得开。

点中间的棋子图标新建一条棋路；点已经长出来的小球，直接进去继续打谱。

![开局径向图](pics/开局径向图.png)

### 打谱

左边是棋盘，右边是光球棋谱树。按象棋规则走子，每一步会变成一个光球；点光球就能跳回那一手。右键可以加分支或写笔记。棋盘照着实体棋盘画：繁体字、炮位和卒林的星位、双线包边。

![中炮对屏风马](pics/中炮对屏风马.png)

工具栏上这几个按钮：

- **输出棋谱树** —— 把整棵树存成一张图片（只有开局棋路有）
- **反转视角** —— 红黑对调，两侧路数跟着翻。黑方永远是 1-9，红方永远是一-九
- **AI 分析** —— 直接分析当前棋盘，不用另外再摆一遍
- **沙盘演练模式** —— 随便试几步，关掉自动回到进入前的局面；试的时候照样能前进后退
- **🔊** —— 落子音效开关

走子、前进后退、点光球跳局面都会响一声落子。一方的将被吃掉时，标题旁边直接标出谁赢了。

点「AI 分析」之后，右边那一列变成「棋谱树 / AI 分析」两个标签页，随时切回去看棋路，切回来结果还在。
